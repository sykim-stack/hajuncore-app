...
> **상태 기준 고정 규칙:** 이 문서는 `master`의 현재 구현 기준 문서다. 과거 방 메시지와 `dev_contexts` 요약은 역사·보조 기록으로 취급하며, 현재 상태는 최신 `master` 코드, 배포 응답, 최신 방 원문 순으로 판정한다. `context_package`는 폐기된 경로이며 현재 컨텍스트 주입 경로가 아니다.
