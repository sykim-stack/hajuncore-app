...
현재 `master`에는 위 기능이 빌드 가능한 상태로 반영되어 있다. 최신 관련 커밋은 `0514214`이며, 프로덕션 빌드와 TypeScript 검증을 통과했다. 다만 Supabase와 외부 AI 키가 런타임 환경에 설정되어야 실제 데이터 조회와 AI 응답이 동작한다.
...
이 조합은 **하준챗을 호출할 때만** 발생한다. 화면 피드가 자동으로 세 마당을 합치거나, 일반적인 AI 호출마다 모든 마당을 무조건 훑는 구조는 아니다.

하준챗의 브라우저 `localStorage`에 남아 있던 이전 문구인 “현재는 채팅 기능이 구현되지 않았습니다”는 감지 시 새 초기 안내 문구로 교체하도록 했다. 과거 세션이 계속 남아 있다면 사용자가 대화를 초기화할 수 있다.
...
넷째, **방 AI 답변의 모델과 정책**을 확정해야 한다. 현재 방의 AI 답변은 최근 방 메시지를 읽고 `openai/gpt-oss-120b`로 응답한다. 답변 저장 주체, `ref_ids` 자동 생성 여부, 사람 확인 전 답변의 상태를 계약으로 정해야 한다.
