...
2. 공급처 캡처 전에 추천 메시지를 선택하여 `ref_ids`로 연결한다. **초기 구현 완료:** BRAINPOOL Core 팝업에서 상품발굴방의 추천 Message를 선택하면 온채널 후보 캡처에 추천 ID가 연결된다.
3. 네이버 시장조사 캡처에 공급처 후보 메시지 ID를 연결한다. **초기 구현 완료:** BRAINPOOL Core 팝업에서 상품발굴방 후보를 선택하면 네이버 조사 캡처에 후보 ID가 연결된다.
