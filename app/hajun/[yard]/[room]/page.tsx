...
export default function RoomPage() {
  ...
  const saveContentNote = async () => {
    ...
    try {
      ...
      const json = await res.json();
      if (json._error) setErrMsg(json._error);
      else { setContent(''); setOkMsg('저장됨'); await load(); }
    } finally { setPosting(false); }
  };

  const advanceWorkStatus = async (kind: 'thumbnail' | 'detail') => {
    if (!room || !currentCode || posting) return;
    ...
    try {
      ...
      const res = await fetch('/api/hajun?action=post_message', {
        ...
        body: JSON.stringify({
          room_id: room.id, author_type: 'human', author_name: authorName || '콘텐츠작업',
          msg_type: 'work_result', content: `${label} 상태: ${CONTENT_STATUS_LABEL[nxt]} (${currentCode})`,
          ref_ids: pickedCandidateId ? [pickedCandidateId] : [],
          metadata: {
      ...
      const res = await fetch('/api/hajun?action=post_message', {
        ...
        body: JSON.stringify({
          ...
          metadata: {
            ...
            channel_ids: {},
            target_malls: Array.isArray(message.metadata?.target_malls) ? message.metadata.target_malls : [],
  ...
  const productSelect = (
    <select
      ...
      onChange={(e) => {
        ...
        const sp0 = found?.metadata?.sale_price;
        if (typeof sp0 === 'number' && sp0 > 0) setSalePrice(String(sp0));
        else if (typeof sp0 === 'string' && sp0.trim()) setSalePrice(sp0.trim());
        ...
        if (code) {
          for (let i = messages.length - 1; i >= 0; i--) {
            ...
            const sp = mm.metadata?.sale_price;
    ...
    >
      <option value="">상품 선택 ({candidateCount})</option>
      {candidates.map((c) => {
        const code = typeof c.metadata?.internal_code === 'string' ? c.metadata.internal_code : '';
              ...
              return (
                <div key={m.id} style={{ ...S.msgCard, ...(m.author_type === 'ai' ? S.msgCardAi : {}), ...(selected ? { borderColor: 'var(--accent)' } : {}) }}>
                  ...
                  <div style={{ marginTop: 8 }}>
                    <button type="button" onClick={() => toggleRef(m.id)}
                      style={{ fontSize: 11, padding: '4px 8px', borderRadius: 6, border: selected ? '1px solid var(--accent)' : '1px solid var(--border)', background: selected ? 'rgba(88,166,255,0.15)' : 'var(--bg3)', color: selected ? 'var(--accent)' : 'var(--text2)', cursor: 'pointer' }}>
                      {selected ? '참조 해제' : '참조'}
                    </button>
                  </div>
                </div>
          ...
          <div style={S.compose}>
            {refIds.length > 0 && (
              <div style={{ marginBottom: 10, display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                {chipMessages.filter((m) => refIds.includes(m.id)).map((m) => (
                  <span key={m.id} style={{ fontSize: 11, padding: '4px 8px', borderRadius: 6, background: 'rgba(88,166,255,0.12)', color: 'var(--accent)', border: '1px solid var(--accent)' }}>
                    {chipLabel(m)}
                  </span>
                ))}
                <button type="button" onClick={clearRefs} style={{ fontSize: 11, padding: '4px 8px', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--bg3)', color: 'var(--text2)', cursor: 'pointer' }}>참조 전체 해제</button>
              </div>
            )}
            ...
            <div style={S.btnRow}>
              <button style={{ ...S.submitBtn, ...(!content.trim() || posting ? S.submitOff : {}) }} disabled={!content.trim() || posting} onClick={submitContext}>
                {posting ? '저장 중...' : '방에 남기기'}
              </button>
              <button style={{ ...S.submitBtn, background: '#39C5CF', ...(aiPosting ? S.submitOff : {}) }} disabled={aiPosting} onClick={askAiContext}>
                {aiPosting ? 'HajunAI 응답 중...' : 'HajunAI 답변'}
              </button>
            </div>
